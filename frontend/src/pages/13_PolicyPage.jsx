...
export default function PolicyPage({ isModal = false, onClose, onAgree }) {
  const { startBgMusic } = useAuth();
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => { 
    setIsVisible(true); 
    
    // تشغيل الموسيقى الخلفية فقط إذا لم تكن modal
    if (!isModal) {
  ...
  // إذا كانت modal، أرجع المحتوى فقط بدون wrapper
  if (isModal) {
    return policyContent;
  }

  // الصفحة العادية (ليست modal)
  return (
